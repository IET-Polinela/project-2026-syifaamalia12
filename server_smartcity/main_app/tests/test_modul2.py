...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_02_laporan_saya_menampilkan_nama_asli(self):
        ...
        for laporan in results:
            ...
            self.assertEqual(
                laporan['reporter'],
                'Warga Anonim',
                "Reporter tetap ditampilkan sebagai Warga Anonim pada response API"
            )
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        # Arrange: siapkan URL detail draf milik Warga B
        url = f'/api/report/{self.draft_milik_b.pk}/'

        # Act: Warga A mencoba membaca draf milik Warga B
        response = self.client.get(url)

        # Assert: sistem harus menyembunyikan draf milik orang lain
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Draf milik pengguna lain seharusnya tidak bisa dibaca dan mengembalikan HTTP 404"
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        # Arrange: simpan data asli draf milik Warga B
        judul_asli = self.draft_milik_b.title
        deskripsi_asli = self.draft_milik_b.description

        # Arrange: siapkan URL dan payload perubahan palsu
        url = f'/api/report/{self.draft_milik_b.pk}/'
        payload = {
            'title': 'Draf Ini Dipaksa Diubah Oleh Warga A',
            'category': self.draft_milik_b.category,
            'description': 'Isi draf ini tidak boleh berubah.',
            'location': self.draft_milik_b.location,
            'incident_date': str(self.draft_milik_b.incident_date),
        ...
        # Assert: sistem menolak dengan HTTP 404
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Modifikasi draf milik pengguna lain seharusnya ditolak dengan HTTP 404"
        )

        # Assert: data asli di database tidak boleh berubah
        self.draft_milik_b.refresh_from_db()
        self.assertEqual(self.draft_milik_b.title, judul_asli)
        self.assertEqual(self.draft_milik_b.description, deskripsi_asli)