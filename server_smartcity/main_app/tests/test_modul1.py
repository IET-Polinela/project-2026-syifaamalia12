...
class AuthenticationTests(APITestCase):
    ...
    # ─────────────────────────────────────────────────────────────────────────
        def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
            ...
            self.assertTrue(
                login_berhasil,
                "User warga harus berhasil login sebelum mengakses dashboard admin"
            )

            # Arrange: tentukan URL dashboard admin
            try:
                url = reverse('dashboard')
            except NoReverseMatch:
                url = '/dashboard/'

            # Act: warga biasa mencoba membuka halaman dashboard admin
            response = self.client.get(url)

            # Assert: warga biasa harus ditolak/dialihkan dengan HTTP 302
            self.assertEqual(
                response.status_code,
                status.HTTP_302_FOUND,
                "Warga biasa yang bukan admin seharusnya tidak bisa mengakses dashboard"
            )